...
// 回復地の計算
int calcRecoverDamage(BattleField *BF, BanishInfo *bani)
{
...
// 回復の表示と最大値を超えた処理
void doRecover(BattleField *BF, BanishInfo *bani)
{
    int nowhp = BF->player->hp;
    int heal = calcRecoverDamage(BF, bani);
    BF->player->hp += heal;
...
// player側の攻撃計算
int calcAttack(BattleField *BF, BanishInfo *bani, int i)
{
    double dame = 0;
...
// player側の攻撃処理
void onAttack(BattleField *BF, BanishInfo *bani)
{
    int dame = 0;
